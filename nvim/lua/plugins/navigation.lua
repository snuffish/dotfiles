...
return {
  ...
  },
  {
    ...
    config = function()
      ...
      vim.utils.map("n", "<localleader>.", dropbar_api.pick, { desc = "Pick symbols in winbar" })
      vim.utils.map(
        "n",
        { "<localleader>.p", "[." },
      ...
      vim.utils.map("n", { "<localleader>.n", "]." }, dropbar_api.select_next_context, { desc = "Select next context" })
    end,
  },
  {
